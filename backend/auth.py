...
def validate_password_strength(password: str) -> bool:
    ...
    # More lenient validation - require at least 3 out of 5 criteria
    criteria_met = sum([has_upper, has_lower, has_digit, has_special, is_long_enough])
...
def register_user(user: User) -> dict:
    ...
    if not validate_password_strength(user.password):
        raise HTTPException(
            status_code=400, 
            detail="Password must be at least 8 characters long and meet at least 3 of these criteria: uppercase letter, lowercase letter, number, special character"
        )
