...
export const SweetProvider = ({ children }) => {
  ...
  const createSweet = async (sweetData) => {
    try {
      ...
      await axios.post(`${API_URL}/api/sweets`, sweetData, {
  ...
  const updateSweet = async (id, sweetData) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${API_URL}/api/sweets/${id}`, sweetData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchSweets();
      return { success: true };
  ...
  const purchaseSweet = async (id, quantity) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/api/sweets/${id}/purchase`, 
  ...
  const restockSweet = async (id, quantity) => {
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API_URL}/api/sweets/${id}/restock`, 
        { quantity: parseInt(quantity) }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
      await fetchSweets();
